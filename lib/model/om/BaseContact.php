...
abstract class BaseContact extends BaseObject  implements Persistent {
	 ...
	 * related GroupToContacts from storage. If this Contact is new, it will return
		...
		if ($this->collGroupToContacts === null) {
			if ($this->isNew()) {
			   $this->collGroupToContacts = array();
				...
				$criteria->add(GroupToContactPeer::CONTACT_ID, $this->id);
	 ...
	 */
	public function getGroupToContactsJoinContactGroup($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ContactPeer::DATABASE_NAME);
		}
		...
		if ($this->collGroupToContacts === null) {
			if ($this->isNew()) {
				...
				$this->collGroupToContacts = GroupToContactPeer::doSelectJoinContactGroup($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
