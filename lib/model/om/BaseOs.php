...
abstract class BaseOs extends BaseObject  implements Persistent {
		...
		if ($this->collHosts === null) {
			if ($this->isNew()) {
			   $this->collHosts = array();
				...
				$criteria->add(HostPeer::OS_ID, $this->getId());
	...
	public function getHostsJoinHostGroup($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseHostPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collHosts === null) {
			if ($this->isNew()) {
				...
				$this->collHosts = HostPeer::doSelectJoinHostGroup($criteria, $con);
			}
		} else {
