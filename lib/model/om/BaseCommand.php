...
abstract class BaseCommand extends BaseObject  implements Persistent {
	 ...
	 * @return     Command The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = CommandPeer::NUM_COLUMNS - CommandPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(CommandPeer::CREATED_AT))
				{
	 ...
	 * Otherwise if this Command has previously been saved, it will retrieve
	 * related Services from storage. If this Command is new, it will return
	...
	public function countServices(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collServices === null) {
			if ($this->isNew()) {
				...
				$count = ServicePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
